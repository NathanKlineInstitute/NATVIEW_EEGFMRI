...
anat=$4
## if use Denoise anat using ANTS
do_denoise=$5
# if multiple scans
num_scans=$6
## if use g-cut
...
## template
template_init_mask=${CCSDIR}/templates/MNI152_T1_1mm_first_brain_mask.nii.gz

if [ $# -lt 8 ];
then
        echo -e "\033[47;35m Usage: $0 anat_dir SUBJECTS_DIR subject anat_name do_denoise num_scans gcut ccs_dir \033[0m"
        exit
fi
...
then
	...
	do
		3drefit -deoblique ${anat_dir}/${anat}${n}_denoise.nii.gz
		mri_convert --in_type nii ${anat}${n}_denoise.nii.gz ${SUBJECTS_DIR}/${subject}/mri/orig/00${n}.mgz
	done	
else
	for (( n=1; n <= ${num_scans}; n++ ))
       	do
