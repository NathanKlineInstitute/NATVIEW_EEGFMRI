...
function collect_data(options)
    ...
    %% Copy raw data
    copy_data_collect(options, options.eye_file_label(2:end), options.raw_dir, sprintf('%s/raw_bids', options.collect_dir))

    %% Copy the preprocessed data
    copy_data_collect(options, options.eye_file_label(2:end), options.preproc_dir, sprintf('%s/preproc_bids', options.collect_dir))
    ...
    aligned_identifier = sprintf('%s_video_aligned_%s', eye_file_parts{1}, eye_file_parts{2});
